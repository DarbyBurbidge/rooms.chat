...
Gets and renders all rooms + the last message sent in each room. This requires 2 requests per room which is excessive. An update should minimize these calls if possible.
...
Displays a chat room. Requires 0 + number of users in chat requests to show user info. Further updates should minimize this as well as add additional information
### JoinModal in [join_room.tsx](frontend/src/join_room.tsx)
A bootstrap modal component which allows users to join a room that they received an invite to. This actually uses a lot of requests 0 + number of users to display images.
